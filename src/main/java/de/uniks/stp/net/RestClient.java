...
public class RestClient {
    ...
    public void login(String username, String password, Callback<JsonNode> callback) {
        ...
        String body = JSONObject.valueToString(jsonObj);
        HttpRequest<?> request = Unirest.post(REST_SERVER_URL + LOGIN_PATH).header("Content-Type", "application/json").body(body);
        sendRequest(request, callback);
    }
    ...
    public void logout(String userKey, Callback<JsonNode> callback) {
        HttpRequest<?> request = Unirest.post(REST_SERVER_URL + LOGOUT_PATH).header("Content-Type", "application/json").header("userKey", userKey);
        sendRequest(request, callback);
    }

    public void getServers(String userKey, Callback<JsonNode> callback) {
        HttpRequest<?> request = Unirest.get(REST_SERVER_URL + SERVER_PATH).header("Content-Type", "application/json").header("userKey", userKey);
        sendRequest(request, callback);
    }

    public void getUsers(String userKey, Callback<JsonNode> callback) {
        HttpRequest<?> request = Unirest.get(REST_SERVER_URL + USERS_PATH).header("Content-Type", "application/json").header("userKey", userKey);
        sendRequest(request, callback);
    }
    ...
    public void getServerUsers(String serverId, String userKey, Callback<JsonNode> callback) {
        ...
        HttpRequest<?> postRequest = Unirest.get(url).header("Content-Type", "application/json").header("userKey", userKey);
    ...
    public void getServerCategories(String serverId, String userKey, Callback<JsonNode> callback) {
        String url = REST_SERVER_URL + SERVER_PATH + "/" + serverId + SERVER_CATEGORIES_PATH;
        HttpRequest<?> postRequest = Unirest.get(url).header("Content-Type", "application/json").header("userKey", userKey);
        sendRequest(postRequest, callback);
    }

    public void getCategoryChannels(String serverId, String categoryId, String userKey, Callback<JsonNode> callback) {
        String url = REST_SERVER_URL + SERVER_PATH + "/" + serverId + SERVER_CATEGORIES_PATH + "/" + categoryId + SERVER_CHANNELS_PATH;
        HttpRequest<?> postRequest = Unirest.get(url).header("Content-Type", "application/json").header("userKey", userKey);
        sendRequest(postRequest, callback);
    }
